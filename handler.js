...
Fitur ini di *disable* !`
    ...
    if (msg) return conn.sendButton(m.chat, danied, msg, `${imgr + 'Accses Danied'}`, [['MENU', '.menu']],m)
    
     let msgg = {
    	unreg: `Error: ${nmsr} Not Found\nRegister First Please`
}[type]
if (msgg) return conn.sendButton(m.chat, `${global.htki} LOGIN ${global.htka}`, msgg, `${imgr + 'Verify'}`, [['Register', '.reg']],m)
}
